"""
Задание 7: Приложение для работы с данными фермерских рынков
================================================================

ПЕРВОНАЧАЛЬНАЯ ВЕРСИЯ:
  - реализована в функциональном стиле (почти без классов, функции
    не изменяют переданные им данные, а возвращают новые структуры);
  - текстовый пользовательский интерфейс типа REPL (Read-Eval-Print Loop);
  - источник данных — файл Export.csv;
  - простые проверки функций (модульные тесты) реализованы в блоке
    if __name__ == '__main__'.

------------------------------------------------------------------
ФОРМАТ ФАЙЛА Export.csv:

Используется реальный формат данных USDA Farmers Market Directory.
Файл содержит 59 столбцов, среди них:

    FMID          - уникальный номер рынка
    MarketName    - название рынка
    Website       - сайт (может быть пустым)
    street, city, County, State, zip - адрес (State - полное название штата,
                    например "Vermont", а не сокращение)
    Season1..4Date/Time - расписание работы (до 4 сезонов)
    x, y          - долгота и широта (могут отсутствовать у части записей)
    Credit, WIC, WICcash, SFMNP, SNAP - какие способы оплаты принимаются (Y/N)
    Organic, Vegetables, Fruits, Meat, Honey, ... - какие товары продаются (Y/N)
    updateTime    - дата последнего обновления записи

Программа не привязана жёстко к списку столбцов: при чтении она
запоминает все столбцы, которые реально есть в файле, и сохраняет
их в том же составе при записи (см. load_markets/save_markets).

Рецензии пользователей хранятся в отдельном файле reviews.csv,
он создаётся автоматически при первом сохранении рецензии:

FMID,FirstName,LastName,Rating,ReviewText,Timestamp
------------------------------------------------------------------

ЗАПУСК:
    python farmers_market_app.py

При запуске сначала выполняются несколько простых самопроверок
основных функций (см. функцию _run_self_tests), а затем открывается
текстовое меню.
"""
...
def _run_self_tests():
    ...
    # рецензии и рейтинг (список рецензий не должен мутировать исходный)
    reviews = []
    new_reviews = add_review(reviews, "1", "Иван", "Иванов", 5, "Отлично!")
    assert reviews == []                       # исходный список не изменился
    assert average_rating(new_reviews, "1") == 5
    assert average_rating(new_reviews, "2") is None

    # расстояние между Спрингфилдом и Чикаго - примерно 170-190 миль
    ...
    # удаление рынка вместе с его рецензиями
