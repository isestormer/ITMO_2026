...
def find_center_by_zip(markets, zip_code):
    ...
    if exact_matches:
        lats = [m["y"] for m in exact_matches]
        lons = [m["x"] for m in exact_matches]
        return (sum(lats) / len(lats), sum(lons) / len(lons), zip_code, True)
    ...
    for m in markets:
        z = m.get("zip")
        if not z or m["x"] is None or m["y"] is None:
            continue
        zips_with_coords.setdefault(z, []).append(m)

    best_zip, best_diff = None, None
    for z in zips_with_coords:
        try:
            diff = abs(int(z) - target)
        except ValueError:
            continue
        if best_diff is None or diff < best_diff:
            best_diff, best_zip = diff, z

    if best_zip is None:
        return None

    group = zips_with_coords[best_zip]
    lats = [m["y"] for m in group]
    lons = [m["x"] for m in group]
    return (sum(lats) / len(lats), sum(lons) / len(lons), best_zip, False)
...
def search_by_zip_radius(markets, zip_code, radius_miles):
    ...
    lat0, lon0, matched_zip, is_exact = center
    result = []
    for m in markets:
    ...
    return result, matched_zip, is_exact
...
def action_search_zip(markets, reviews):
    ...
    search_result = search_by_zip_radius(markets, zip_code, radius)
    if search_result is None:
        print("В базе данных нет ни одного рынка с координатами — поиск по индексу невозможен.")
        return

    result, matched_zip, is_exact = search_result
    if not is_exact:
        print(f"Индекса {zip_code} нет среди рынков в базе. "
              f"Ищу от ближайшего известного индекса {matched_zip} (местоположение приблизительное).")
...
def action_sort(markets, reviews):
    ...
    if key == "distance":
        ...
        center = find_center_by_zip(markets, zip_code)
        if center is None:
            print("В базе нет ни одного рынка с координатами, сортировка по расстоянию невозможна.")
            return
        lat0, lon0, matched_zip, is_exact = center
        ref_point = (lat0, lon0)
        if not is_exact:
            print(f"Индекса {zip_code} нет среди рынков в базе. "
                  f"Использую координаты ближайшего известного индекса {matched_zip} (приблизительно).")
